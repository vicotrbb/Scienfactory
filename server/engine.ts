...
Use research_guide to design controls and checks. For parameter sensitivity, convergence, simulation replicates or ablations, use run_study to freeze the protocol before execution, supply factors and finite metrics, preserve failures, and automatically show comparison charts. Use compare_study for paired case comparisons with explicit sampling assumptions; it preserves failed checks and refuses missing pairs. Use trace_artifact to distinguish a generating computation from a later inspection. For reproducibility, use reproduce_execution on an actual new execution record; exact reruns are distinct from independent scientific replication.
