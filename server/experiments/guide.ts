export const RESEARCH_GUIDE = {
  workflow: [
    ...
    'Use compare_study to compare two parameter cases over paired repeats, with explicit independence assumptions. Use trace_artifact to inspect the recorded generators and inputs of a result. Use independent implementations or analytic cases to check the computation. Use reproduce_execution to replay an actual execution under its recorded image and inputs and compare exact file bytes and stdout. Reproduction is not independent replication; timestamps and nondeterminism can cause harmless byte differences.',
